...
const AuthForm = () => {
    ...
    const [user, setUser] = useState(() => JSON.parse(localStorage.getItem('dreamcodedUser')));
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch('https://dreamcoded.com/api/auth.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
            const result = await res.json();

            if (result.success) {
                if (mode === 'login') {
                    const formattedUser = { ...result.user, avatar_url: formatAvatarURL(result.user.avatar_url) };
